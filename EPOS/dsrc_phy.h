...
class DSRC_Phy: public NIC_Common
{
private:
    typedef uint8 Reg8;
...
public:
    ...
    static const unsigned int CCA_TX_GAP = 160/24;
    static const unsigned int TURNAROUND_TIME = 192/24;
    static const unsigned int BYTE_RATE = 750000;//  31250; // bytes per second
    ...
    public:
        ...
        friend std::ostream & operator<<(std::ostream & db, const Phy_Frame & f) {
            db << "{d=" << f._data << "}";
            return db;
        }
    ...
    public:
        ...
        public:
            ...
            friend std::ostream & operator<<(std::ostream & db, const Frame_Control & fc) {
    ...
    public:
        ...
        : _frame_control(type), _sequence_number(uniform(0, 4294967295)), _dst_pan_id(PAN_ID_BROADCAST) {};

        Header(const Type & type, const Address & src, const Address & dst)
        : _frame_control(type), _sequence_number(uniform(0, 4294967295)), _dst_pan_id(PAN_ID_BROADCAST), _dst(dst), _src(src) { ack_request(dst != broadcast()); }
        ...
        friend std::ostream & operator<<(std::ostream & db, const Header & h) {
    ...
    {
    public:
        static const unsigned int MTU = DSRC_Phy::MTU - sizeof(Header) - sizeof(CRC);
        typedef unsigned char Data[MTU];
    ...
    public:
        ...
        friend std::ostream & operator<<(std::ostream & db, const Frame & f) {
    ...
    class Ack_Frame : public Header::Frame_Control {
    public:
        Ack_Frame(const Reg8 & seq) : Frame_Control(ACK, 0, 0, 0, 0, 0, 0, 0), _sequence_number(seq) {}
    ...
    typedef _UTIL::Buffer<void, Phy_Frame, void, NIC_Common::Metadata> Buffer;

    // Observers of a protocol get a also a pointer to the received buffer
    typedef _UTIL::Data_Observer<Buffer, Type> Observer;
    typedef _UTIL::Data_Observed<Buffer, Type> Observed;
    ...
    struct Statistics: public NIC_Common::Statistics
    {
        ...
        friend std::ostream & operator<<(std::ostream & db, const Statistics & s) {
            db << "{rxp=" << s.rx_packets
               << ",rxb=" <<  s.rx_bytes
