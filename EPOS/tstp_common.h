...
//#define OLDMICROFRAME
//#define EXPLICIT_SYNC_ONLY
...
public:
    ...
    class Microframe
    {
        ...
        // Bit 0            1     16      24     56    72
        //     +------------+------+-------+------+-----+
        //     | all listen |  id  | count | hint | crc |
        //     +------------+------+-------+------+-----+
        // Bits       1        15     8        32    16
    public:
        static const unsigned int ID_MAX = 0x7fff;
        static const unsigned int COUNT_MAX = 0x0ff;

        Microframe() {}

        Microframe(bool all_listen, const Frame_ID & id, const MF_Count & count, const Hint & hint = 0)
        : _al_id(htoles(all_listen | ((id & ID_MAX) << 1))), _count(count), _hint(hint), _crch(0) {}

        MF_Count count() const { return _count; }
        void count(const MF_Count & c) { _count = c; }
        ...
        Frame_ID id() const { return (letohs(_al_id) >> 1); }

        void id(const Frame_ID & id) {
            _al_id = htoles(((id & ID_MAX) << 1) | all_listen());
        }

        void all_listen(bool all_listen) {
            if(all_listen)
                _al_id = htoles((letohs(_al_id) | 0x01));
            else
                _al_id = htoles((letohs(_al_id) & ~0x01));
        }

        bool all_listen() const { return letohs(_al_id) & 0x01; }

        Hint hint() const { return letohl(_hint); }
        void hint(const Hint & h) { _hint = htolel(h); }
    ...
    private:
        unsigned short _al_id; // All listen, ID
    ...
    public:
        ...
        : _config((S & 0x03) << 6 | tr << 5 | (t & 0x03) << 3 | (v & 0x07)), _confidence(c), _time(ot), _origin(o),
#ifndef EXPLICIT_SYNC_ONLY
          _last_hop_time(0),
#endif
          _last_hop(l) {}
        ...
        Time last_hop_time() const;
        void last_hop_time(const Time & t);
#else
        ...
        friend std::ostream & operator<<(std::ostream & db, const _Header & h) {
            db << "{v=" << h.version() - V0 << ",t=" << ((h.type() == INTEREST) ? 'I' :  (h.type() == RESPONSE) ? 'R' : (h.type() == COMMAND) ? 'C' : 'T') << ",co=" << h._confidence << ",tr=" << h.time_request() << ",s=" << h.scale() << ",ot=" << h._time << ",o=" << h._origin <<
#ifndef EXPLICIT_SYNC_ONLY
                    ",lt=" << h._last_hop_time <<
#endif
                    ",l=" << h._last_hop << "}";
    ...
    public:
        ...
        : Header(RESPONSE, 0, 0, 0, Coordinates(0,0,0), Coordinates(0,0,0)), _unit(unit), _error(error), _expiry(expiry), _auth(0), _crc(0) {
    ...
    public:
        ...
        : Control(KEEP_ALIVE, 0, 0, 0, Coordinates(0,0,0), Coordinates(0,0,0)), _crc(0) { }
