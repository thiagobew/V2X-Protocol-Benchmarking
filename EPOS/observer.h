...
public:
    Observed() {
        //db<Observers>(TRC) << "Observed() => " << this << std::endl;
    }
    ~Observed() {
        //db<Observers>(TRC) << "~Observed(this=" << this << ")" << std::endl;
    }
...
protected:
    Observer(): _link(this) {
        //db<Observers>(TRC) << "Observer() => " << this << std::endl;
    }

public:
    ~Observer() {
        //db<Observers>(TRC) << "~Observer(this=" << this << ")" << std::endl;
    }
...
{
    //db<Observers>(TRC) << "Observed::attach(obs=" << o << ")" << std::endl;
...
{
    //db<Observers>(TRC) << "Observed::detach(obs=" << o << ")" << std::endl;
...
inline bool Observed::notify()
{
    ...
    //db<Observers>(TRC) << "Observed::notify()" << std::endl;

    for(Element * e = _observers.head(); e; e = e->next()) {
        //db<Observers>(INF) << "Observed::notify(this=" << this << ",obs=" << e->object() << ")" << std::endl;
...
public:
    Conditionally_Observed() {
        //db<Observers>(TRC) << "Conditionally_Observed() => " << this << std::endl;
    }
    ~Conditionally_Observed() {
        //db<Observers>(TRC) << "~Conditionally_Observed(this=" << this << ")" << std::endl;
    }

    virtual void attach(Conditional_Observer<T> * o, T c) {
        //db<Observers>(TRC) << "Conditionally_Observed::attach(o=" << o << ",c=" << c << ")" << std::endl;
    ...
    virtual void detach(Conditional_Observer<T> * o, T c) {
        //db<Observers>(TRC) << "Conditionally_Observed::detach(obs=" << o << ",c=" << c << ")" << std::endl;
    ...
    virtual bool notify(T c) {
        ...
        //db<Observers>(TRC) << "Conditionally_Observed::notify(cond=" << hex << c << ")" << std::endl;
...
protected:
    Conditional_Observer(): _link(this) {
        //db<Observers>(TRC) << "Conditionally_Observer() => " << this << std::endl;
    }

public:
    ~Conditional_Observer() {
        //db<Observers>(TRC) << "~Conditionally_Observer(this=" << this << ")" << std::endl;
    }
...
public:
    Data_Observed() {
        //db<Observers>(TRC) << "Data_Observed<T>() => " << this << std::endl;
    }

    ~Data_Observed() {
        //db<Observers>(TRC) << "~Data_Observed<T>(this=" << this << ")" << std::endl;
    }

    virtual void attach(Data_Observer<T1, T2> * o, T2 c) {
        //db<Observers>(TRC) << "Data_Observed<T>::attach(obs=" << o << ",cond=" << c << ")" << std::endl;
    ...
    virtual void detach(Data_Observer<T1, T2> * o, T2 c) {
        //db<Observers>(TRC) << "Data_Observed<T>::detach(obs=" << o << ",cond=" << c << ")" << std::endl;
    ...
    virtual bool notify(T2 c, T1 * d) {
        ...
        //db<Observers>(TRC) << "Data_Observed<T>::notify(this=" << this << ",cond=" << c << ")" << std::endl;

        for(Element * e = _observers.head(); e; e = e->next()) {
            if(e->rank() == c) {
                //db<Observers>(INF) << "Data_Observed<T>::notify(this=" << this << ",obs=" << e->object() << ")" << std::endl;
...
protected:
    Data_Observer(): _link(this) {
        //db<Observers>(TRC) << "Data_Observer<T>() => " << this << std::endl;
    }

public:
    ~Data_Observer() {
        //db<Observers>(TRC) << "~Data_Observer<T>(this=" << this << ")" << std::endl;
    }
...
public:
    Data_Observed() {
        //db<Observers>(TRC) << "Data_Observed() => " << this << std::endl;
    }

    ~Data_Observed() {
        //db<Observers>(TRC) << "~Data_Observed(this=" << this << ")" << std::endl;
    }

    virtual void attach(Data_Observer<T1, void> * o) {
        //db<Observers>(TRC) << "Data_Observed::attach(obs=" << o << ")" << std::endl;
    ...
    virtual void detach(Data_Observer<T1, void> * o) {
        //db<Observers>(TRC) << "Data_Observed::detach(obs=" << o << ")" << std::endl;
    ...
    virtual bool notify(T1 * d) {
        ...
        //db<Observers>(TRC) << "Data_Observed::notify(this=" << this << ")" << std::endl;
...
protected:
    Data_Observer(): _link(this) {
        //db<Observers>(TRC) << "Data_Observer() => " << this << std::endl;
    }

public:
    ~Data_Observer() {
        //db<Observers>(TRC) << "~Data_Observer(this=" << this << ")" << std::endl;
    }
